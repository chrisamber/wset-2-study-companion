...
The chat model is called through the Vercel AI Gateway rather than a direct provider SDK — a plain `"anthropic/claude-sonnet-4-6"` string passed straight to `streamText`, with zero provider-specific code. Vercel's docs describe zero-config OIDC auth for this in production; testing the live deployment showed that didn't hold here, so `AI_GATEWAY_API_KEY` is set explicitly in production too, same as local dev — a reminder to verify a vendor's documented default against actual deployed behavior.
